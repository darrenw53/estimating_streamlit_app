...
import logic
from dxf_plate import parse_dxf_plate_parts
...
def page_plate() -> None:
    ...
    with st.expander("DXF Batch Import (Plate Only)", expanded=False):
        ...
        min_bbox_area = st.number_input(
            "Min detected part bbox area (in²)",
            min_value=0.0,
            value=0.5,
            step=0.1,
            help="Filters out tiny loops often caused by drawing annotations. Set to 0 to disable.",
        )
        ...
        if uploaded:
            ...
            for f in uploaded:
                try:
                    ...
                    for p in parts:
                        if float(min_bbox_area) > 0:
                            if (float(p.bbox_w_in) * float(p.bbox_l_in)) < float(min_bbox_area):
                                continue
                        detected_rows.append(
                            {
                                "Source DXF": f.name,
                                "Part Name": p.part_name,
                                "Width (in)": float(p.bbox_w_in),
                                "Length (in)": float(p.bbox_l_in),
                                "True Cut Perimeter (in)": float(p.cut_perimeter_in),
                                "Hole Count": int(p.hole_count),
                                "Total Hole Circumference (in)": float(p.hole_circumference_in),
