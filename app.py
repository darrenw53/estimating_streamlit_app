...
APP_TITLE = "Estimating Calculator(V5)"
...
def _import_estimate_from_csv(uploaded_file) -> Tuple[bool, str]:
    """
    Import an exported estimate CSV (from this app) and repopulate st.session_state["estimate_parts"].

    The imported CSV is treated as the source of input rows; summary totals will recompute from rows.
    """
    try:
    ...
    # Basic validation: must have at least estimation type and part name columns
    required_cols = {"Estimation Type", "Part Name", "Quantity"}
    missing = [c for c in required_cols if c not in df.columns]
    if missing:
        return False, f"CSV is missing required columns: {', '.join(missing)}"

    # Convert rows to list[dict], cleaning NaNs
    rows: List[Dict[str, Any]] = []
    for rec in df.to_dict(orient="records"):
        clean = {k: _nan_to_none(v) for k, v in rec.items()}

        # Normalize a few common fields
        # Quantity should be int if possible
        try:
            if clean.get("Quantity") is not None and str(clean.get("Quantity")).strip() != "":
                clean["Quantity"] = int(float(clean["Quantity"]))
        except Exception:
            pass

        # Estimation Type should be canonical string
        if clean.get("Estimation Type") is not None:
            clean["Estimation Type"] = str(clean["Estimation Type"]).strip()

        if clean.get("Part Name") is not None:
            clean["Part Name"] = str(clean["Part Name"]).strip()

        rows.append(clean)

    # Replace current estimate
    st.session_state["estimate_parts"] = rows

    # Clear yield caches (optional) - user can rerun optimizers if desired
    st.session_state["plate_yield_results"] = {}
    st.session_state["structural_yield_results"] = {}

    return True, f"Imported {len(rows)} line item(s)."
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.write("—")
        with st.expander("Resume / Import from CSV", expanded=False):
            up = st.file_uploader("Upload an exported estimate CSV", type=["csv"], key="resume_csv_uploader")
            st.caption("This will replace the current estimate and repopulate the Summary from the imported line items.")
            if st.button("Import CSV", type="primary", key="import_csv_btn", disabled=(up is None)):
                ok, msg = _import_estimate_from_csv(up)
                if ok:
                    st.success(msg)
                    st.rerun()
                else:
